...
import com.demo.linhao.festival_sms.MainActivity;
...
public class IntroScreenActivity extends AppCompatActivity{
            ...
            public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
                ...
                if (position == TOTAL_PAGES - 2 && positionOffset > 0) {
                    if (isOpaque) {
                    	...
                    	viewpager.setBackgroundColor(getResources().getColor(R.color.highlighted_text_material_light));
                        isOpaque = false;
                    }
                } else {
                    if (!isOpaque) {
    ...
    /**
     *    这里必须手动调用finish去关掉IintroScreenActivity
     */
    @Override
	...
	public class CrossfadePageTransformer implements ViewPager.PageTransformer {
        ...
        public void transformPage(View page, float position) {
            ...
            if (0 <= position && position < 1) {
                Log.d("SYS",pageWidth+":--------:"+pageWidth * (-position)+"---------"+position);
